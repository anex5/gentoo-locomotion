	...
	for var; do
		case "${var}" in
			EPYTHON)
				...
				;;
			PYTHON)
				_python_wrapper_setup ${impl} PYTHON
				debug-print "${FUNCNAME}: PYTHON = ${PYTHON}"
				;;
			PYTHON_SITEDIR)
				_python_wrapper_setup ${impl} _PYTHON
				PYTHON_SITEDIR=$(
					"${_PYTHON}" - <<-EOF || die
						import sysconfig
...
# @FUNCTION: p/var/overlay/gentoo-locomotion/eclass/python-utils-r1.eclassython_get_includedir
# @USAGE: [<impl>]
# @DESCRIPTION:
...
python_doheader() {
	...
	local d=${includedir#${ESYSROOT-${SYSROOT}${EPREFIX}}}
