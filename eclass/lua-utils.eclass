...
_lua_wrapper_setup() {
	...
	if [[ ! -x ${workdir}/bin/lua ]]; then
		...
		# Lua interpreter
		ln -s "${ESYSROOT-${SYSROOT}${EPREFIX}}"/usr/bin/${ELUA} "${workdir}"/bin/lua || die

		# Lua compiler, or a stub for it in case of luajit
		if [[ ${ELUA} == luajit ]]; then
			# Just in case
			ln -s "${ESYSROOT-${SYSROOT}${EPREFIX}}"/bin/true "${workdir}"/bin/luac || die
		else
			ln -s "${ESYSROOT-${SYSROOT}${EPREFIX}}"/usr/bin/${ELUA/a/ac} "${workdir}"/bin/luac || die
		fi
