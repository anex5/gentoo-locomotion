...
IUSE="avif contrib contribcvv contribdnn contribfreetype contribhdf contribovis contribsfm contribxfeatures2d \
cuda cudnn debug dnnsamples +eigen examples flatbuffers +features2d ffmpeg gdal gflags glog gphoto2 gstreamer gtk3 \
ieee1394 jpeg jpeg2k lapack lto non-free onnx opencl openexr opengl openmp opencvapps png +python qt5 qt6 tesseract testprograms \
tbb tiff vaapi v4l vtk vulkan wayland webp xine video_cards_intel ${CUDA_TARGETS_COMPAT[@]/#/cuda_targets_}"
...
pkg_pretend() {
	if use cuda && [[ -z "${CUDA_GENERATION}" ]] ; then
