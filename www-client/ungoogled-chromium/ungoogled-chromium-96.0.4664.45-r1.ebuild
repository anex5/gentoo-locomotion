...
+system-freetype +system-fontconfig +system-re2 -system-libevent +system-icu +system-ffmpeg +system-harfbuzz +system-libdrm -system-libvpx +system-dav1d \
...
src_prepare() {
	...
	local PATCHES=(
		...
		"${p}/chromium-88-less-blacklist-nonsense.patch"

		# Extra patches taken from Mageia
		#"${p}/chromium-norar.patch" ungoogled core/debian
	...
	use !clang && (
		eapply "${p}/chromium-gcc-disabled-warnings.patch"
		eapply "${p}/chromium-gcc-va_args.patch"
	)
	...
	use system-ffmpeg && (
		...
		use system-dav1d && eapply "${p}/chromium-79-system-dav1d.patch"
	...
	use system-freetype && (
		...
		eapply "${p}/chromium-96-system-freetype.patch"
	)

	#use system-harfbuzz && eapply "${p}/chromium-95-harfbuzz-3.patch"
	...
	use vaapi && (
		...
		use vdpau && eapply "${p}/vdpau-support-r4.patch"
		use wayland && eapply "${p}/igalia/0001-ozone-add-va-api-support-to-wayland.patch"
	)
	...
	use wayland && (
		...
		eapply "${p}/igalia/0001-ozone-wayland-fix-re-initialization-of-WBMG.patch"
	...
	use v4l && (
		...
		eapply "${p}/chromium-v4l2-remove-legacy-kernel-headers.patch"
	)
	...
	eapply "${p}/0002-Remove-the-GN-settings-done-for-clang-that-conflict-.patch"
	eapply "${p}/0007-Delete-compiler-options-not-available-in-release-ver.patch"
	...
	use "elibc_musl" && (
		...
		eapply "${p}/fix-bits-stdint-intn-include.patch"
	)
	...
	use system-harfbuzz && keeplibs+=( third_party/harfbuzz-ng/utils ) || keeplibs+=( third_party/harfbuzz-ng )
	use system-freetype && keeplibs+=( third_party/freetype )
...
src_configure() {
	...
	use system-re2 && gn_system_libraries+=( re2 )
	use system-fontconfig && gn_system_libraries+=( fontconfig )
	use system-ffmpeg && gn_system_libraries+=( ffmpeg opus openh264 )
	...
	use system-dav1d && gn_system_libraries+=( dav1d )
	use system-libdrm && gn_system_libraries+=( libdrm )
	use system-harfbuzz && gn_system_libraries+=( harfbuzz-ng )
	use system-freetype && gn_system_libraries+=( freetype )
	...
	myconf_gn+=(
		...
		"use_system_freetype=$(usetf system-freetype)"
	...
	myconf_gn+=(
		...
		"linux_link_libspeechd=$(usetf atk)"
		"linux_link_libbrlapi=$(usetf atk)"
	...
	if ! use custom-cflags; then
		...
		if use x86; then
			filter-flags "-g* -msse2"
			append-ldflags "-Wl,--no-keep-files-mapped"
		fi
