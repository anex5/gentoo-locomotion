...
MOZ_FIREFOX_FILE="waterfox-current"

if [[ ! -r ${MOZ_LIB_DIR}/waterfox-current/${MOZ_FIREFOX_FILE} ]]; then
	if [[ ! -r ${SECONDARY_LIB_DIR}/waterfox-current/${MOZ_FIREFOX_FILE} ]]; then
		echo "Error: ${MOZ_LIB_DIR}/waterfox-current/${MOZ_FIREFOX_FILE} not found" >&2
		if [[ -d $SECONDARY_LIB_DIR ]]; then
			echo "       ${SECONDARY_LIB_DIR}/waterfox-current/${MOZ_FIREFOX_FILE} not found" >&2
		fi
		exit 1
	fi
	MOZ_LIB_DIR="$SECONDARY_LIB_DIR"
fi
MOZILLA_FIVE_HOME="${MOZ_LIB_DIR}/waterfox-current"
MOZ_EXTENSIONS_PROFILE_DIR="${HOME}/.mozilla/extensions/{ec8030f7-c20a-464f-9b0e-13a3a9e97384}"
MOZ_PROGRAM="${MOZILLA_FIVE_HOME}/${MOZ_FIREFOX_FILE}"
DESKTOP_FILE="waterfox-current"
...
if @DEFAULT_WAYLAND@ && [[ -z ${MOZ_DISABLE_WAYLAND} ]]; then
	if [[ -n "$WAYLAND_DISPLAY" ]]; then
		DESKTOP_FILE="waterfox-current-wayland"
		export MOZ_ENABLE_WAYLAND=1
	fi
elif [[ -n ${MOZ_DISABLE_WAYLAND} ]]; then
	DESKTOP_FILE="waterfox-current-x11"
fi
