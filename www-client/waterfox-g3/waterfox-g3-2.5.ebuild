...
MOZ_SRC_BASE_URI="https://archive.mozilla.org/pub/firefox/releases/78.13.0esr"
...
IUSE="+clang cpu_flags_arm_neon dbus debug eme-free geckodriver +gmp-autoupdate
	...
	+system-libvpx +system-webp wayland wifi kde"
...
src_prepare() {
	...
	rm "${WORKDIR}/firefox-patches/0038-bmo-1667736-Update-packed_simd-to-compile-on-Rust-1..patch" || die
	rm "${WORKDIR}/firefox-patches/0040-bmo-1680166-Return-EFAULT-when-given-a-null-path-to-.patch" || die
	rm "${WORKDIR}/firefox-patches/0041-bmo-1680166-GCC-is-smarter-than-clang-so-ignore-the-.patch" || die
	eapply "${WORKDIR}/firefox-patches"

	# Allow user to apply any additional patches without modifing ebuild
	eapply_user

	sed -e "/check_prog\(\'XARGS\', \(\'xargs\',\)\)/ d" "${S}"/moz.configure
