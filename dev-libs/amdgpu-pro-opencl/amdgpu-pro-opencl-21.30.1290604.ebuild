...
pkg_nofetch() {
	...
	einfo "Please download Radeon Software for Linux version ${pkgver} for Ubuntu 20.04.333rom"
	einfo "    ${HOMEPAGE}"
...
multilib_src_unpack() {
	...
	use amf && unpack_deb "${S}/amf-amdgpu-pro_${MY_PV}_${deb_abi:-${ABI}}.deb"
	popd >/dev/null || die
}
...
multilib_src_install() {
	...
	use opencl && (
		dolib.so "opt/${SUPER_PN}/lib/${dir_abi}"/libamdocl*
		dolib.so "opt/amdgpu/lib/${dir_abi}"/*
	)

	use amf && dolib.so "opt/${SUPER_PN}/lib/${dir_abi}"/libamfrt*
