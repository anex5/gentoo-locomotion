...
#PATCHES=(
	#"${FILESDIR}/coinor-clp-1.17.6-fix-config-libs-detection.patch"
#)
...
src_configure() {
	...
	if use glpk; then
		myeconfargs+=(
			--with-amd-incdir="$($(tc-getPKG_CONFIG) --variable=includedir AMD)"
			--with-amd-lib="$($(tc-getPKG_CONFIG) --libs AMD)"
	...
	if use sparse; then
		myeconfargs+=(
			--with-amd-incdir="$($(tc-getPKG_CONFIG) --variable=includedir AMD)"
			--with-amd-lib="$($(tc-getPKG_CONFIG) --libs AMD)"
			--with-cholmod-incdir="$($(tc-getPKG_CONFIG) --variable=includedir CHOLMOD)"
			--with-cholmod-lib="$($(tc-getPKG_CONFIG) --libs CHOLMOD)"
			--disable-amd-libcheck
	...
	if use metis; then
		myeconfargs+=(
			--with-metis-incdir="$($(tc-getPKG_CONFIG) --variable=includedir metis)"
