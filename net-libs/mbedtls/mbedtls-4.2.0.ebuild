...
SRC_URI="https://github.com/Mbed-TLS/mbedtls/releases/download/${P}/${P}.tar.bz2 -> ${P}.gh.tar.bz2"

LICENSE="|| ( Apache-2.0 GPL-2+ )"
SLOT="4/17.23.9" # ffmpeg subslot naming: SONAME tuple of {libmbedcrypto.so,libmbedtls.so,libmbedx509.so}
...
PATCHES=(
	"${FILESDIR}/mbedtls-4.2.0-allow-install-headers-to-different-location.patch"
	"${FILESDIR}/mbedtls-4.2.0-add-version-suffix-for-all-installable-targets.patch"
	"${FILESDIR}/mbedtls-4.2.0-add-version-suffix-for-pkg-config-files.patch"
	"${FILESDIR}/mbedtls-4.2.0-slotted-version.patch"
)
...
multilib_src_configure() {
	local mycmakeargs=(
		...
		-DCMAKE_INSTALL_INCLUDEDIR="include/mbedtls${PV%%.*}"
